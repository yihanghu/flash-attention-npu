"""Standalone v3 varlen benchmark adapted from the repository test."""
...
def flash_attn_varlen_func(
    ...
    seqused_q=None, seqused_k=None, softmax_scale=None, causal=False, qv=None,
    q_descale=None, k_descale=None, v_descale=None, window_size=(-1, -1),
    attention_chunk=0, softcap=0.0, num_splits=1, pack_gqa=None,
    deterministic=False, sm_margin=0, return_attn_probs=False,
):
    if qv is not None or any(item is not None for item in (seqused_q, seqused_k, q_descale, k_descale, v_descale)):
        raise NotImplementedError("Optional v3 features are absent from the repository golden cases")
