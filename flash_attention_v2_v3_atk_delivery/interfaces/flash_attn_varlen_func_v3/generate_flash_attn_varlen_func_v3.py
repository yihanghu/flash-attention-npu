"""Cross-input constraints for v3 ``flash_attn_varlen_func``."""
...
@GENERATOR_REGISTRY.register("flash_attn_varlen_func_v3_constraints")
class FlashAttnVarlenFuncV3Constraints(CaseGenerator):
    def after_case_config(self, case_config):
        ...
        inputs["qv"].shape = [total_q, heads_q, head_dim]
        inputs["qv"].range_values = "default"
        for name in ("seqused_q", "seqused_k", "q_descale", "k_descale", "v_descale"):
